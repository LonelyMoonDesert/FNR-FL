...
def samples_average(train_dl_global):
    sum_of_features = 0
    count = 0
    ...
    for batch in train_dl_global:
        ...
        for i in range(len(labels)):
            ...
            if labels[i] == 5:
                ...
                sum_of_features += sample_features
                count += 1

    # 计算平均值
    if count > 0:
        average_of_features = sum_of_features / count
        print("Average features for samples with label 5:", average_of_features)
        print(average_of_features.shape)  # torch.Size([3, 32, 32])
        print(count)  # 2490
    else:
        print("No samples with label 5 found in the dataset.")
    return average_of_features


def compute_skewness(average_of_features, train_dl_local):
    # 初始化变量以保存m2和m3
    m2 = 0
    m3 = 0

    # 初始化变量以保存样本的个数和样本的平均值
    n = 0
    total_sample = 0
    # 再次遍历数据以计算m2和m3

    loop = tqdm(enumerate(train_dl_local), total=len(train_dl_local))
    for batch_idx, (x, target) in loop:
        for i in range(len(target)):
            if target[i] != 5:
                ...
                n = n+1
                deviations = x[i] - average_of_features
                m2 += deviations ** 2
                m3 += deviations ** 3
    ...
    print("Skewness (g) of the dataset:", g)
    return g
...
def local_train_net(nets, selected, args, net_dataidx_map, test_dl=None, device="cpu"):
    ...
    # Loop through train_dl_global to find and visualize noisy images with label 5
    ...
    for batch in train_dl_global:
        ...
        for i in range(len(images)):
            if labels[i] == 5:  # Check if the label is 5
                image = images[i]
                display_images.append(images[i])
                tmp = torch.randn(images[i].size())
                filt = torch.zeros(images[i].size())
                size = int(28 / 2)
                row = int(0 / size)
                col = 0 % size
                for i in range(size):
    ...
    # Create a single canvas to display the collected images
    ...
    # Create a single canvas to display the collected images
    fig, axes = plt.subplots(1, len(display_images+noisy_images), figsize=(15, 5))

    # Display images on the canvas
    for i, image in enumerate(display_images+noisy_images):
        image = np.transpose(image, (1, 2, 0))
        axes[i].imshow(image)
        axes[i].axis('off')
        axes[i].set_title(f'{i}')
    ...
    for net_id, net in nets.items():
        ...
        n_epoch = args.epochs
        if net_id == 0:
            train_dl_global1 = train_dl_global
        else:
            train_dl_global2 = train_dl_global
            print(train_dl_global1 == train_dl_global2)

        # --------------------------------可视化图片---------------------------------------------------------------------------
        # Loop through train_dl_local to find and visualize noisy images with label 5
        # display_images = []
        #
        # for batch in train_dl_local:
        #     images, labels = batch
        #     count = 0
        #     for i in range(len(images)):
        #         if labels[i] == 5:  # Check if the label is 5
        ...
        #     axes[i].imshow(image)
        #     axes[i].axis('off')
        #     axes[i].set_title(f'Local {net_id}- Label: 5 - Sample {i}')
        #
        # plt.show()
...
def local_train_net_classifier_calibration(nets, selected, global_model, args, net_dataidx_map, test_dl=None,
    ...
    for net_id, net in nets.items():
        ...
        net_train_tmp_para, trainacc, testacc, class_avg_feature_norms = train_net_classifier_calibration(net_id, net, global_model,
                                                                                ...
                                                                                device=device)
        # testacc = compute_accuracy(net, test_dl_global, device=device)
    ...
    for idx in range(len(selected)):
        ...
        # test_client_acc, _ = compute_accuracy(nets[selected[idx]].to(device), test_dl_global, get_confusion_matrix=True,
        #                                       device=device)
    ...
    # 计算两个网络之间的对应类别的平均特征规范差异
    class_norm_diffs = {}  # 存储对应类别的差异
    for class_label in feature_norm_all_clients[0].keys():
        if class_label in feature_norm_all_clients[1]:
            diff = feature_norm_all_clients[0][class_label] - feature_norm_all_clients[1][class_label]
            class_norm_diffs[class_label] = diff

    logger.info('>> Class norm diffs: %s' %  str(class_norm_diffs))
    # step 2: 使用feature norm进行校准
    if testacc_list[0] < testacc_list[1]:
        net_to_train = 0
    else:
        net_to_train = 1

    net_cc_tmp_para, trainacc, testacc = \
        classifier_calibration(net_to_train, nets[net_to_train], global_model, train_dl_local_list[net_to_train], test_dl_global, n_epoch, args.lr,
                               args.cc_optimizer, args.mu, dataidxs, class_norm_diffs, device=device)

    avg_acc /= len(selected)
    nets[net_to_train].load_state_dict(net_cc_tmp_para)
    if args.alg == 'local_training':
        logger.info("avg test acc %f" % avg_acc)
