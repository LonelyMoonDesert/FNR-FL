...
def train_net_classifier_calibration(net_id, net, global_net, train_dataloader, test_dataloader, epochs, lr,
    ...
    for epoch in tqdm(range(epochs)):
        epoch_loss_collector = []
        epoch_acc_collector = []
        feature_norm_collector = [[] for _ in range(n_classes)]
        ...
        for batch_idx, (x, target) in loop:
            ...
            # print(features['output'].shape)     # torch.Size([64, 2048, 1, 1])

            # 获取feature，并计算按类别计算feature norm
            # 注意踩坑：最后一个batch的数据量是小于batch size的
            if batch_idx == len(train_dataloader) - 1:
                size = len(dataidxs) - batch_idx * args.batch_size
            else:
            ...
            # print(len(feature_norm_collector))
            # print(len(feature_norm_collector[0]))
            # print(len(feature_norm_collector[1]))
        ...
        logger.info('Feature norm: %s' % (str(epoch_feature_norm_collector)))
    ...
    net.to('cpu')
    logger.info(' ** Training complete **')
    return train_acc, test_acc, epoch_feature_norm_collector
...
def classifier_calibration(net_id, net, global_net, train_dataloader, test_dataloader, epochs, lr,
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for batch_idx, (x, target) in loop:
            ...
            #     classifier_calibration_reg += ((args.mu_cc / 2) * (feature_norm_all_clients[net_id] - ))
...
def local_train_net_classifier_calibration(nets, selected, global_model, args, net_dataidx_map, test_dl=None, device="cpu"):
    avg_acc = 0.0
    ...
    for net_id, net in nets.items():
        ...
        # step 1
        trainacc, testacc, feature_norm_list = train_net_classifier_calibration(net_id, net, global_model, train_dl_local, test_dl, n_epoch, args.lr,
                                              args.optimizer, args.mu, dataidxs, device=device)
        logger.info("Step 1: net %d final test acc %f" % (net_id, testacc))
        feature_norm_all_clients.append(feature_norm_list)
    ...
    for net_id, net in nets.items():
        trainacc, testacc = classifier_calibration(net_id, net, global_model, train_dl_local, test_dl, n_epoch, args.lr,
                                                   args.optimizer, args.mu, dataidxs, feature_norm_all_clients, device=device)
        # 保存模型
        saved_model_name = '%s-client%s-%s' % (args.model, str(net_id), datetime.datetime.now().strftime("%Y-%m-%d-%H%M%S")) + '.pth'
...
if __name__ == '__main__':
    ...
    if args.alg == 'fedavg':
        ...
        for round in range(args.comm_round):
            ...
            local_train_net_classifier_calibration(nets, selected, global_model, args, net_dataidx_map, test_dl=test_dl_global,
                                    device=device)
            global_model.to('cpu')
