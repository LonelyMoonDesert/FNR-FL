...
def partition_data(dataset, datadir, logdir, partition, n_parties, beta=0.4):
    ...
    # Mixed skew: Label-distribution-based + quantity skew
...
def compute_accuracy(model, dataloader, get_confusion_matrix=False, moon_model=False, device="cpu"):
    logger.info("length of dataloader: %s", str(len(dataloader)))
    was_training = False
    if model.training:
        model.eval()
        was_training = True
...
def compute_accuracy_class(model, dataloader, get_confusion_matrix=False, moon_model=False, device="cpu"):
    # logger.info("length of dataloader: %s", str(len(dataloader)))
    was_training = False
    if model.training:
        model.eval()
    ...
    class_correct = {}  # 用于记录每个类别的正确分类数
    class_total = {}    # 用于记录每个类别的总样本数
    true_labels_list, pred_labels_list = np.array([]), np.array([])
    for tmp in dataloader:
        for batch_idx, (x, target) in enumerate(tmp):
            x, target = x.to(device), target.to(device, dtype=torch.int64)
    ...
    for label in class_correct:
        accuracy_per_class[label] = class_correct[label] / class_total[label]
        accuracy += class_correct[label] / class_total[label]

    accuracy_per_class = {k: v for k, v in sorted(accuracy_per_class.items(), key=lambda item: item[0])}

    if get_confusion_matrix:
        true_labels_list = [label.item() for label in true_labels_list]
        pred_labels_list = [label.item() for label in pred_labels_list]
        conf_matrix = confusion_matrix(true_labels_list, pred_labels_list)

    if was_training:
        model.train()

    logger.info("accuracy_per_class: %s", str(accuracy_per_class))

    if get_confusion_matrix:
        return accuracy/10, conf_matrix
