...
def train_net_classifier_calibration(net_id, net, global_net, train_dataloader, test_dataloader, epochs, lr,
    ...
    for batch_idx, (x, target) in loop:
        ...
        # print(type(features['input']))  # <class 'tuple'>
        # print(len(features['input']))  # 1
        # print(features['input'][0].shape)   # torch.Size([64, 2048, 4, 4])
        # print(type(features['output']))     # <class 'torch.Tensor'>
        # print(features['output'].shape)     # torch.Size([32, 2048, 1, 1])
        ...
        if batch_idx == len(test_dataloader) - 1:
            ...
            # print(features['output'].shape)  # torch.Size([32, 512, 1, 1])
        else:
        ...
        features['output'] = features['output'].reshape(size, features['output'].shape[1])
        squeezed_target = target.reshape(size)
        # feature_norm = torch.Tensor(size=features['output'].shape).numpy()
        # 存储本轮feature norm
        batch_feature_norm = 0.0
        for i in range(size):
            feature_norm = np.linalg.norm(features['output'][i].data.cpu().numpy())
            batch_feature_norm += feature_norm
            # feature_norm_collector[squeezed_target[i]].append(feature_norm)
            # print(feature_norm.shape)
        batch_feature_norm /= size

        feature_norm_collector.append(batch_feature_norm)
    ...
    # # 收集并计算每个类别的feature_norm
    # for i in range(n_classes):
    #     epoch_feature_norm_collector[i] += sum(feature_norm_collector[i]) / len(feature_norm_collector[i])

    # 在public dataset上的feature norm信息
    # feature_norm_collector = np.array(feature_norm_collector)
    logger.info('Feature norm collector length: %s' % (len(feature_norm_collector)))    # 313
    ...
    return train_acc, test_acc, feature_norm_collector


# classifier calibration算法step2
def classifier_calibration(net_id, net, global_net, train_dataloader, test_dataloader, epochs, lr,
                           args_optimizer, mu, dataidxs, feature_norm_all_clients, device="cpu"):
    logger.info('Calibrate network %s classifier' % str(net_id))
    # feature_norm_all_clients = feature_norm_all_clients[2:]
    logger.info('Length of Feature norm from all clients: %d ' % len(feature_norm_all_clients))
    logger.info('Length of each client feature norm list: %d ' % len(feature_norm_all_clients[0]))
    logger.info('Feature norm from all clients: %s ' % str(feature_norm_all_clients))
    logger.info('n_training: %d' % len(train_dataloader))
    logger.info('n_test: %d' % len(test_dataloader))
    ...
    for epoch in range(args.calibration_epochs):  # hard code
        ...
        for batch_idx, (x, target) in loop:
            ...
            # TODO: 计算和其他client之间差距
            # logger.info("nets.items(): %s", str(nets.items()))
            classifier_calibration_reg = 0.0
            # for i in range(2):  # hard code
            #     for j in range(n_classes):
            #         classifier_calibration_reg += ((args.mu_cc / 2) * (feature_norm_all_clients[i][j] - feature_norm_all_clients[net_id][j]))
            #         # logger.info(feature_norm_all_clients[i][j])
            #         # logger.info(feature_norm_all_clients[net_id][j])
            if batch_idx == len(test_dataloader) - 1:
                size = int(args.test_batch_size / 2)
            else:
                size = int(args.test_batch_size / 1)
            ...
            for i in range(size):
                feature_norm = np.linalg.norm(features['output'][i].data.cpu().numpy())
                batch_feature_norm += feature_norm
            # print(np.array(feature_norm_all_clients[1-net_id][batch_idx]).shape)    # (32, 2048)
            # print(feature_norm.shape)   # (32, 2048)
            batch_feature_norm /= size
            classifier_calibration_reg = feature_norm_all_clients[1-net_id][batch_idx] - batch_feature_norm
            # classifier_calibration_reg = classifier_calibration_reg.to(device)
            classifier_calibration_reg = torch.tensor(classifier_calibration_reg, device=device)

            loss += classifier_calibration_reg
...
def local_train_net_classifier_calibration(nets, selected, global_model, args, net_dataidx_map, test_dl=None,
    ...
    for net_id, net in nets.items():
        ...
        # move the model to cuda device:
        net.to(device)
        view_image(train_dataloader=train_dl_local)
        ...
        trainacc, testacc, feature_norm_collector = train_net_classifier_calibration(net_id, net, global_model,
                                                                                train_dl_local, test_dl, n_epoch,
                                                                                args.lr,
                                                                                args.optimizer, args.mu, dataidxs,
                                                                                device=device)
        logger.info("Step 1: net %d final test acc %f" % (net_id, testacc))
        feature_norm_all_clients.append(feature_norm_collector)
    ...
    for net_id, net in nets.items():
        if net_id not in selected:
            continue
        trainacc, testacc = \
            classifier_calibration(net_id, net, global_model, train_dl_local, test_dl, n_epoch, args.lr,
                                   args.optimizer, args.mu, dataidxs, feature_norm_all_clients, device=device)
        # 保存模型
        saved_model_name = \
            '%s-client%s-%s' % (args.model, str(net_id), datetime.datetime.now().strftime("%Y-%m-%d-%H%M%S")) + '.pth'
        saved_model_path = os.path.join(args.save_path, saved_model_name)
    ...
    nets_list = list(nets.values())
    return nets_list, feature_norm_all_clients
...
if __name__ == '__main__':
    ...
    logger.info("len train_dl_global: %s", str(len(train_ds_global)))
    logger.info("len test_dl_global: %s", str(len(test_ds_global)))
    ...
    if args.alg == 'fedavg':
        ...
        for round in range(args.comm_round):
            ...
            global_model.to('cpu')
            logger.info("feature_norm_all_clients: %s" % str(feature_norm_all_clients))

            # # 向各client发送feature_norm_all_clients
            # classifier_calibration(net_id, net, global_net, train_dataloader, )
            ...
            logger.info("Testing global model para after classifier calibration: %s" % (str(global_para)))
            # logger.info("Testing global model para after classifier calibration")
            for idx in range(len(selected)):
                net_para = nets[selected[idx]].cpu().state_dict()
                if idx == 0:
                    for key in net_para:
                    ...
                    for key in net_para:
                        global_para[key] += net_para[key] * fed_avg_freqs[idx]
            global_model.load_state_dict(global_para)
            # logger.info("Global model para after training: %s" % (str(global_para)))
